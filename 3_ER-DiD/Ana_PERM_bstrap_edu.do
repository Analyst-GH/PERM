...
global y1 yearseduc_FOB
global y2 yearseduc_FOB_2
global y3 yearseduc_FOB_3
global y4 yearseduc_FOB_4
...
*PERM DiD
* We are going to fill the matrix with main estimates plus 200 bsamples and then use this to provide BS standard errors
*Define matrix to fill, and number of bootstrap repititions 
matrix simu_PERM_EDU=J(201,520,.)
* First do it all for main estimates (k=1), then for k=2-201 we bsample
forvalues k=1(1)1 {
di `k'
 {	
...
cap matrix simu_PERM_EDU[`k',480]=C_11 
cap matrix simu_PERM_EDU[`k',481]=C_TT
...
* Now we do it for k=2-201 with bsample
...
* save the main estimates and bsamples as a data file
drop* 
svmat double simu_PERM_EDU
cap save "$output\sim_PERM_EDUV2.dta", replace
clear all
...
forvalues i = 442(1)461 {
rename simu_PERM_EDU`i' yz_TT_`i'
sum yz_TT_`i' if _n!=1
gen yz_TT_se_`i'=r(sd)
sum yz_TT_`i' if _n==1
replace yz_TT_`i'=r(mean)
gen yz_TT_95LB_`i'=r(mean)-1.96*yz_TT_se_`i'
gen yz_TT_95UB_`i'=r(mean)+1.96*yz_TT_se_`i'
}

forvalues i = 462(1)480 {
	local j=`i'+31
gen yz_VAR_TT_`i' = simu_PERM_EDU`i' - simu_PERM_EDU`j'  
sum yz_VAR_TT_`i' if _n!=1
gen yz_VAR_TT_se_`i'=r(sd)
sum yz_VAR_TT_`i' if _n==1
replace yz_VAR_TT_`i'=r(mean)
gen yz_VAR_TT_95LB_`i'=r(mean)-1.96*yz_VAR_TT_se_`i'
gen yz_VAR_TT_95UB_`i'=r(mean)+1.96*yz_VAR_TT_se_`i'
}

forvalues i = 462(1)480 {
rename simu_PERM_EDU`i' yz_adj_TT_`i'
sum yz_adj_TT_`i' if _n!=1
gen yz_adj_TT_se_`i'=r(sd)
sum yz_adj_TT_`i' if _n==1
replace yz_adj_TT_`i'=r(mean)
gen yz_adj_TT_95LB_`i'=r(mean)-1.96*yz_adj_TT_se_`i'
gen yz_adj_TT_95UB_`i'=r(mean)+1.96*yz_adj_TT_se_`i'
}

forvalues i = 493(1)511 {
rename simu_PERM_EDU`i' yz_2_DD_`i'
sum yz_2_DD_`i' if _n!=1
gen yz_2_DD_se_`i'=r(sd)
sum yz_2_DD_`i' if _n==1
replace yz_2_DD_`i'=r(mean)
gen yz_2_DD_95LB_`i'=r(mean)-1.96*yz_2_DD_se_`i'
gen yz_2_DD_95UB_`i'=r(mean)+1.96*yz_2_DD_se_`i'
}
...
reshape long 	y1_TT_ y1_TT_se_ y1_TT_95LB_ y1_TT_95UB_ 
				...
				yz_TT_ yz_TT_se_ yz_TT_95LB_ yz_TT_95UB_
				yz_adj_TT_ yz_adj_TT_se_ yz_adj_TT_95LB_ yz_adj_TT_95UB_
				yz_VAR_TT_ yz_VAR_TT_se_ yz_VAR_TT_95LB_ yz_VAR_TT_95UB_
				yz_2_DD_ yz_2_DD_se_ yz_2_DD_95LB_ yz_2_DD_95UB_
				, i(id) j(time)
;
# delimit cr;
* Y1
...
* YZ
replace time=time-450 if time>442 & time<451
replace time=time-450 if time>450 & time<463
replace time=time-469 if time>462 & time<481
replace time=time-500 if time>492 & time<512
...
tw  (rcap y1_TT_95LB_ y1_TT_95UB_ time if id==1 & time<9, color(gs9)) (scatter y1_TT_ time if id==1 & time<9, color(ebblue) connect(l) msymbol(Th)), legend(order(2 "E[Y]" 1 "E[Y] 95% CI") col(1) pos(11) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Years of Education") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Y1_EDU_A.pdf", replace

* Y squared vs y_adj squared event study figure
tw  (rcap y2_TT_95LB_ y2_TT_95UB_ time if id==1 & time<9, color(gs9)) (scatter y2_TT_ time if id==1 & time<9, connect(l) msymbol(Oh)) (rcap y2_adj_TT_95LB_ y2_adj_TT_95UB_ time if id==1 & time<9, color(gs4)) (scatter y2_adj_TT_ time if id==1 & time<9, color(ebblue) connect(l) msymbol(Th)), legend(order(4 "E[Y{superscript:2}] Treatment Effect" 3 "E[Y{superscript:2}] TE 95% CI" 2 "E[Y{superscript:2}] DiD" 1 "E[Y{superscript:2}] DiD 95% CI" ) col(1) pos(5) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Years of Education{superscript:2}") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Y2_EDU_TT_A.pdf", replace

* y_adj squared vs E[Y]^2 event study figure
tw  (rcap y2_adj_TT_95LB_ y2_adj_TT_95UB_ timemi if id==1 & time<9, color(gs9)) (scatter y2_adj_TT_ timemi if id==1 & time<9, color(ebblue) connect(l) msymbol(Oh))  (scatter y1_2_DD_ timepl if id==1 & time<9, color(red) connect(l) msymbol(Th)), legend(order(2 "E[Y{superscript:2}] " 1 "E[Y{superscript:2}]  95% CI" 3 "No Variance Effect" ) col(1) pos(5) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Parameter Treatment Effect on the Treated") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Y2_EDU_TT_vsEYTT_A.pdf", replace


* Variance event study figure
tw  (rcap y2_VAR_TT_95LB_ y2_VAR_TT_95UB_ timemi if id==1 & time<9, color(gs9)) (scatter y2_VAR_TT_ timemi if id==1 & time<9, color(ebblue) connect(l) msymbol(Oh))  , legend(order(2 "Variance Treatment Effect " 1 "Variance TE 95% CI"  ) col(1) pos(1) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Parameter Treatment Effect on the Treated") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Var_EDU_TT_A.pdf", replace



* Y cubed vs y3_adj cubed event study figure
tw   (rcap y3_TT_95LB_ y3_TT_95UB_ time if id==1 & time<9, color(gs4)) (scatter y3_TT_ time if id==1 & time<9,  connect(l) msymbol(Th)) (rcap y3_adj_TT_95LB_ y3_adj_TT_95UB_ time if id==1 & time<9, color(gs9)) (scatter y3_adj_TT_ time if id==1 & time<9, color(ebblue) connect(l) msymbol(Oh)), legend(order(4 "E[Y{superscript:3}] Treatment Effect" 3 "E[Y{superscript:3}] TE 95% CI" 2 "E[Y{superscript:3}] DiD" 1 "E[Y{superscript:3}] DiD 95% CI" ) col(1) pos(11) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Years of Education{superscript:3}") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Y3_EDU_adjusted_A.pdf", replace

* y_adj cubed vs skewness remainder event study figure
tw  (rcap y3_adj_TT_95LB_ y3_adj_TT_95UB_ timemi if id==1 & time<9, color(gs9)) (scatter y3_adj_TT_ timemi if id==1 & time<9, color(ebblue) connect(l) msymbol(Oh))  (scatter y2_2_DD_ timepl if id==1 & time<9, color(red) connect(l) msymbol(Th)), legend(order(2 "E[Y{superscript:3}]" 1 "E[Y{superscript:3}] 95% CI" 3 "No Skewness Effect"  ) col(1) pos(5) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Parameter Treatment Effect on the Treated") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Y3_EDU_versusY12_A.pdf", replace
...
graph export "$outtab\Skew_EDU_TT_A.pdf", replace


* Y quadruple vs y4_adj  event study figure
tw   (rcap y4_TT_95LB_ y4_TT_95UB_ time if id==1 & time<9, color(gs4)) (scatter y4_TT_ time if id==1 & time<9,  connect(l) msymbol(Th)) (rcap y4_adj_TT_95LB_ y4_adj_TT_95UB_ time if id==1 & time<9, color(gs9)) (scatter y4_adj_TT_ time if id==1 & time<9, color(ebblue) connect(l) msymbol(Oh)), legend(order(4 "E[Y{superscript:4}] Treatment Effect" 3 "E[Y{superscript:4}] TE 95% CI" 2 "E[Y{superscript:4}] DiD" 1 "E[Y{superscript:4}] DiD 95% CI" ) col(1) pos(11) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Years of Education{superscript:4}") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Y4_EDU_adjusted_A.pdf", replace

* y_adj ^4 vs kurtosis remainder event study figure
tw  (rcap y4_adj_TT_95LB_ y4_adj_TT_95UB_ timemi if id==1 & time<9, color(gs9)) (scatter y4_adj_TT_ timemi if id==1 & time<9, color(ebblue) connect(l) msymbol(Oh))  (scatter y3_2_DD_ timepl if id==1 & time<9, color(red) connect(l) msymbol(Th)), legend(order(2 "E[Y{superscript:4}]" 1 "E[Y{superscript:4}]  95% CI" 3 "No Kurtosis Effect"  ) col(1) pos(5) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Parameter Treatment Effect on the Treated") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Y4_EDU_versusY123_A.pdf", replace

* kurtosis event study figure
tw  (rcap y4_VAR_TT_95LB_ y4_VAR_TT_95UB_ timemi if id==1 & time<9, color(gs9)) (scatter y4_VAR_TT_ timemi if id==1 & time<9, color(ebblue)  connect(l) msymbol(Oh))  , legend(order(2 "Kurtosis Treatment Effect " 1 "Kurtosis TE 95% CI"  ) col(1) pos(7) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Parameter Treatment Effect on the Treated") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Kurt_EDU_TT_A.pdf", replace



* ZY
* Y squared vs y_adj squared event study figure
tw  (rcap yz_TT_95LB_ yz_TT_95UB_ time if id==1 & time<9, color(gs9)) (scatter yz_TT_ time if id==1 & time<9, connect(l) msymbol(Oh)) (rcap yz_adj_TT_95LB_ yz_adj_TT_95UB_ time if id==1 & time<9, color(gs4)) (scatter yz_adj_TT_ time if id==1 & time<9, color(ebblue) connect(l) msymbol(Th)), legend(order(4 "E[YZ] Treatment Effect" 3 "E[YZ] TE 95% CI" 2 "E[YZ] DiD" 1 "E[YZ] DiD 95% CI" ) col(1) pos(11) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Years of Education{superscript:2}") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\YZ_EDU_TT_A.pdf", replace

* y_adj squared vs E[Y]^2 event study figure
tw  (rcap yz_adj_TT_95LB_ yz_adj_TT_95UB_ timemi if id==1 & time<9, color(gs9)) (scatter yz_adj_TT_ timemi if id==1 & time<9, color(ebblue) connect(l) msymbol(Oh))  (scatter yz_2_DD_ timepl if id==1 & time<9, color(red) connect(l) msymbol(Th)), legend(order(2 "E[YZ] " 1 "E[YZ]  95% CI" 3 "No Covariance Effect" ) col(1) pos(11) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Parameter Treatment Effect on the Treated") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\YZ_EDU_TT_vs_EZY_A.pdf", replace


* Covariance event study figure
tw  (rcap yz_VAR_TT_95LB_ yz_VAR_TT_95UB_ timemi if id==1 & time<9, color(gs9)) (scatter yz_VAR_TT_ timemi if id==1 & time<9, color(ebblue) connect(l) msymbol(Oh))  , legend(order(2 "Covariance Treatment Effect " 1 "Covariance TE 95% CI"  ) col(1) pos(11) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Parameter Treatment Effect on the Treated") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Cov_EDU_TT_A.pdf", replace
