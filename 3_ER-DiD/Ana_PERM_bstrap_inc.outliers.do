...
/*PERM DiD - uses "did_multiplegt_GHDP.ado" to estimate DiD of raw moments and correction terms for PERM DiD
 ...
 - This specific file is the same as main results file, but drops extreme most outliers in each g,t cell to see if results change 
...
* drop outliers by birth cohort and reform year (293 obs)
 egen running=group(cohort firstcohort60), label(running, replace)

 forvalues i=1(1)293 {
sum inc if running==`i'
replace inc=. if inc==r(max) & running==`i'
}
...
cap save "$output\sim_PERM_INCV2_Outlier.dta", replace
clear all
use "$output\sim_PERM_INCV2_Outlier.dta", clear
gen id=_n
...
tw  (rcap y1_TT_95LB_ y1_TT_95UB_ time if id==1 & time<9, color(gs9)) (scatter y1_TT_ time if id==1 & time<9, color(ebblue) connect(l) msymbol(Th)), legend(order(2 "E[Y]" 1 "E[Y] 95% CI") col(1) pos(11) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Income") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Y1_INC_A_outlier.pdf", replace

* Y squared vs y_adj squared event study figure
tw  (rcap y2_TT_95LB_ y2_TT_95UB_ time if id==1 & time<9, color(gs9)) (scatter y2_TT_ time if id==1 & time<9, connect(l) msymbol(Oh)) (rcap y2_adj_TT_95LB_ y2_adj_TT_95UB_ time if id==1 & time<9, color(gs4)) (scatter y2_adj_TT_ time if id==1 & time<9, color(ebblue) connect(l) msymbol(Th)), legend(order(4 "E[Y{superscript:2}] Treatment Effect" 3 "E[Y{superscript:2}] TE 95% CI" 2 "E[Y{superscript:2}] DiD" 1 "E[Y{superscript:2}] DiD 95% CI" ) col(1) pos(11) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Income{superscript:2}") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Y2_INC_TT_A_outlier.pdf", replace

* y_adj squared vs E[Y]^2 event study figure
tw  (rcap y2_adj_TT_95LB_ y2_adj_TT_95UB_ timemi if id==1 & time<9, color(gs9)) (scatter y2_adj_TT_ timemi if id==1 & time<9, color(ebblue)  connect(l) msymbol(Oh))  (scatter y1_2_DD_ timepl if id==1 & time<9, color(red) connect(l) msymbol(Th)), legend(order(2 "E[Y{superscript:2}] " 1 "E[Y{superscript:2}]  95% CI" 3 "Variance - E[Y{superscript:2}]" ) col(1) pos(11) ring(0) ) xtitle("Birth Years till First Treated Birth Cohort (t=0)") ytitle("Parameter Treatment Effect on the Treated") xsize(4) ysize(3.5) xlabel(-6(2)8)
graph export "$outtab\Y2_INC_TT_vsEYTT_A_outlier.pdf", replace
...
graph export "$outtab\Skew_INC_TT_A_outlier.pdf", replace
