...
*PC=1 for PC & PC=0 for mac
global PC 0

if $PC==1 {
* PC
global datause "G:\Shared drives\Inequality Decomposition\Monte Carlo Sim\BS11"
global outtab "G:\Shared drives\Inequality Decomposition\Monte Carlo Sim\MCS_BS11Figures"
}
if $PC==0 {
* mac
...
* Append all replications across sample sizes
...
forvalues j=1001(1)11000 {
	display `j'
capture append using "$datause/r_MCS1_nSS`k'_`j'.dta"
cap drop if Var_cr1==.
}
...
forvalues j=1(1)1000 {
	display `j'
capture append using "$datause/r_MCS1_nSS10000_`j'.dta"
drop if Var_cr9==.
}
replace sampsize=10000 if sampsize==.
forvalues j=1(1)2000 {
	display `j'
capture append using "$datause/r_MCS1_nSS100000_`j'.dta"
drop if Var_cr9==.
}
...
replace sampsize_2=11 if sampsize==1000000
bysort sampsize: gen FIRST=(_n==1)


*** level of y
sca TE_Kurtosis_std =  41.727796
sca TE_Kurtosis =  97.726706
sca TE_Skewness_std =  3.1681685
sca TE_Skewness =  6.8928078
sca TE_Mean =  1.1019412
sca TE_Variance =  1.2852445
...
gen Var_cr21_covPC=(Var_cr109<TE_Variance & TE_Variance<Var_cr110)
sum Var_cr21_covPC
* PERM linearisation s.e
gen Var_cr11_cov=(abs(Var_cr13-TE_Variance)<(1.6449*Var_cr21))
sum Var_cr11_cov
* PERM bootstrap s.e
gen Var_cr22_cov=(abs(Var_cr13-TE_Variance)<(1.6449*Var_cr29))
sum Var_cr22_cov
* IPW bootstrap s.e
gen Var_cr7_cov=(abs(Var_cr78-TE_Variance)<(1.6449*Var_cr83))
sum Var_cr7_cov
* IPW PC bootstrap 90% CI
gen Var_cr7_covPC=(Var_cr121<TE_Variance & TE_Variance<Var_cr122)
sum Var_cr7_covPC

bysort sampsize: egen coverage_linear=mean(Var_cr11_cov)
bysort sampsize: egen coverage_BS=mean(Var_cr22_cov)
bysort sampsize: egen coverage_PC=mean(Var_cr21_covPC)
bysort sampsize: egen coverage_W=mean(Var_cr7_cov)
bysort sampsize: egen coverage_WPC=mean(Var_cr7_covPC)
...
graph tw (line coverage_linear sampsize if FIRST==1) ///
		...
		ylab(0.0(0.2)1) xscale(log) xlabel(250 1000 10000 100000 , format(%15.0gc)) ///
...
graph export "$outtab/Var_Coverage_Y.pdf", replace	


* Skewness coverage
* PERM bootstrap s.e
gen S_P_CovBS=(abs(Var_cr15-TE_Skewness)<(1.6449*Var_cr31))
sum S_P_CovBS
* PERM PC bootstrap CI
gen S_P_CovPC=(Var_cr113<TE_Skewness & TE_Skewness<Var_cr114)
sum S_P_CovPC
* IPW bootstrap s.e
gen S_W_covBS=(abs(Var_cr164-TE_Skewness)<(1.6449*Var_cr166))
sum S_W_covBS
* IPW PC bootstrap 90% CI
gen S_W_covPC=(Var_cr184<TE_Skewness & TE_Skewness<Var_cr185)
sum S_W_covPC
...
graph export "$outtab/S_Coverage_Y.pdf", replace		

* Standardised skewness coverage
* PERM bootstrap s.e
gen Var_cr16_cov=(abs(Var_cr16-TE_Skewness_std)<(1.6449*Var_cr32))
sum Var_cr16_cov
* PERM PC bootstrap CI
gen Var_cr16_covPC=(Var_cr115<TE_Skewness_std & TE_Skewness_std<Var_cr116)
sum Var_cr16_covPC
* IPW bootstrap s.e
gen Var_cr80_cov=(abs(Var_cr80-TE_Skewness_std)<(1.6449*Var_cr85))
sum Var_cr80_cov
* IPW PC bootstrap 90% CI
gen Var_cr80_covPC=(Var_cr125<TE_Skewness_std & TE_Skewness_std<Var_cr126)
sum Var_cr80_covPC

bysort sampsize: egen coverage_BSSS=mean(Var_cr16_cov)
bysort sampsize: egen coverage_PCSS=mean(Var_cr16_covPC)
bysort sampsize: egen coverage_WSS=mean(Var_cr80_cov)
bysort sampsize: egen coverage_WPCSS=mean(Var_cr80_covPC)
cap bysort sampsize: gen FIRST=(_n==1)
...
graph export "$outtab/SS_Coverage_Y.pdf", replace		
...
bysort sampsize: egen Avg_M_PERM=mean(Var_cr11)
bysort sampsize: egen Avg_M_IPW =mean(Var_cr77)
...
graph export "$outtab/M_BIAS_Y.pdf", replace	

** Variance
bysort sampsize: egen Avg_Var_PERM=mean(Var_cr12)
bysort sampsize: egen Avg_Var_PERM2=mean(Var_cr13)
bysort sampsize: egen Avg_Var_IPW =mean(Var_cr78)

local truth=TE_Variance
di `truth'
graph tw (line Avg_Var_PERM sampsize if FIRST==1) ///
			(line Avg_Var_PERM2 sampsize if FIRST==1) ///
		 (line Avg_Var_IPW sampsize if FIRST==1, lpattern(dash)), ///
		ylab(.48(.3)2.08) xscale(log) xlabel(250 1000 10000 100000 , format(%15.0gc)) ///
		yline(`truth', lpattern(dash)) legend(order(1 "PERM " 2 "PERM bias corrected" 3 "IPW " ) ///
		position(1) ring(0))  xtitle("Sample Size (log scale)") ytitle("PTT")  graphregion(margin(r+4)) xsize(4) ysize(4) 

graph export "$outtab/Var_BIAS_Y.pdf", replace		

** Un-Standardised skewness
cap drop Avg_S_PERM
bysort sampsize: egen Avg_S_PERM=mean(Var_cr15)
bysort sampsize: egen Avg_S_PERM2=mean(Var_cr23)
bysort sampsize: egen Avg_S_IPW=mean(Var_cr164)
bysort sampsize: egen Avg_S_Unfeasible=mean(Var_cr160)
...
graph tw (line Avg_S_PERM sampsize if FIRST==1) ///
		...
		ylab(-50(20)50) xscale(log) xlabel(250 1000 10000 100000 , format(%15.0gc)) ///
		yline(`truth') legend(order(1 "PERM " 2 "PERM bias corrected" 3 "IPW " ) ///
		position(1) ring(0))  xtitle("Sample Size (log scale)") ytitle("PTT")  graphregion(margin(r+4)) xsize(4) ysize(4) 

graph export "$outtab/S_BIAS_Y.pdf", replace	


** Standardised skewness
bysort sampsize: egen Avg_SS_PERM=mean(Var_cr16)
bysort sampsize: egen Avg_SS_IPW=mean(Var_cr80)

local truth=TE_Skewness_std
di `truth'
graph tw (line Avg_SS_PERM sampsize if FIRST==1) ///
(line Avg_SS_IPW sampsize if FIRST==1), ///
		ylab(1.4(.8)5) xscale(log) xlabel(250 1000 10000 100000 , format(%15.0gc)) ///
		yline(`truth') legend(order(1 "PERM "  2 "IPW "  ) ///
		position(1) ring(0))  xtitle("Sample Size (log scale)") ytitle("PTT")  graphregion(margin(r+4)) xsize(4) ysize(4) 

graph export "$outtab/SS_BIAS_Y.pdf", replace	


***************************************************************
** ROOT MEAN SQUARE ERROR FIGURES
** Variance
gen MSE_Var=(Var_cr13-TE_Variance)^2
bysort sampsize: egen average_MSE_Var=mean(MSE_Var)
gen RMSE_Var=average_MSE_Var^0.5

gen MSE_Var_P=(Var_cr12-TE_Variance)^2
bysort sampsize: egen average_MSE_Var_P=mean(MSE_Var_P)
gen RMSE_Var_P=average_MSE_Var_P^0.5

gen MSE_Var_IPW=(Var_cr78-TE_Variance)^2
...
graph export "$outtab/S_RMSE_Y.pdf", replace	

** Standardised skewness
gen MSE_SS=(Var_cr16-TE_Skewness_std)^2
bysort sampsize: egen average_MSE_SS=mean(MSE_SS)
gen RMSE_SS=average_MSE_SS^0.5

gen MSE_SS_IPW=(Var_cr80-TE_Skewness_std)^2
...
graph tw (line RMSE_SS sampsize if FIRST==1) ///
		...
		ylab(#5) xscale(log) xlabel(250 1000 10000 100000 , format(%15.0gc)) ///
		legend(order(1 "PERM " 2 "IPW ") ///
		position(1) ring(0)) xtitle("Sample Size (log scale)") ytitle("RMSE")  graphregion(margin(r+4)) xsize(4) ysize(4) 

graph export "$outtab/SS_RMSE_Y.pdf", replace		
